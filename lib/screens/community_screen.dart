...
class _CommunityScreenState extends State<CommunityScreen> {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          // Bot Speech bubble and FAB in bottom right
          Positioned(
            bottom: 16.h,
            right: 16.w,
            child: Row(
                  ...
                  GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            'HI ${_userName.split(' ').first.toUpperCase()}!',
                            style: GoogleFonts.inter(
                              fontSize: 9.sp,
  ...
  void _showAiHelperDialog() {
    ...
    List<Map<String, String>> chatLog = [
      {
        'role': 'assistant',
        'text': 'Hi $_userName! I am your EduSphere Assistant. How can I help you in your school community today?'
      }
    ];
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setModalState) => AlertDialog(
          ...
          title: Row(
            children: [
              Icon(Icons.assistant_rounded, color: widget.theme.primary),
              SizedBox(width: 8.w),
              Text('EduSphere Assistant', style: GoogleFonts.outfit(fontWeight: FontWeight.w800, fontSize: 18.sp)),
            ],
          ),
          content: SizedBox(
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    SizedBox(width: 8.w),
                    IconButton(
                      icon: Icon(Icons.send, color: widget.theme.primary),
                      onPressed: () {
                        ...
                        setModalState(() {
                          chatLog.add({'role': 'user', 'text': query});
                        });

                        // Simulated automated replies based on keywords
                        Future.delayed(const Duration(milliseconds: 600), () {
                          String reply = 'That\'s interesting! You can check the announcements tab or create a community post to collaborate.';
                          final lq = query.toLowerCase();
                          if (lq.contains('attendance')) {
                            reply = 'You can track attendance records under your Academic overview dashboard.';
                          } else if (lq.contains('class') || lq.contains('subject')) {
                            reply = 'Use the Classes and Subjects tabs in Academic Overview to manage classroom subdivisions.';
                          } else if (lq.contains('post') || lq.contains('blog') || lq.contains('community')) {
                            reply = 'You can create a new post in this Community feed by tapping "+ Create Post" at the top right.';
                          } else if (lq.contains('exam') || lq.contains('schedule')) {
                            reply = 'Go to the Academic -> Exams & Results tab to check upcoming schedules and download report cards.';
                          }

                          if (mounted) {
                            setModalState(() {
                              chatLog.add({'role': 'assistant', 'text': reply});
                            });
                          }
                        });
                      },
