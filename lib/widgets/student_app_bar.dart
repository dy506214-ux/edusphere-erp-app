...
class StudentAppBar extends StatefulWidget implements PreferredSizeWidget {
  final String title;
  const StudentAppBar({super.key, this.title = 'EduSphere'});
...
class _StudentAppBarState extends State<StudentAppBar> with TickerProviderStateMixin {
  bool _isMuted = false;
  ...
  void _onAnnouncementEvent(dynamic data) {
    debugPrint('⚡ [Socket Event] Student Announcement change event: $data');
    if (mounted) {
      _loadAnnouncements();
    }
  }
  ...
  void _onNotificationEvent(dynamic data) {
    debugPrint('⚡ [Socket Event] Student Notification received: $data');
    if (mounted) {
      _loadNotifications();
  ...
  Future<void> _loadAnnouncements() async {
    ...
    try {
      ...
      debugPrint('Error loading announcements in student app bar: $e');
      if (mounted) {
  ...
  Future<void> _loadNotifications() async {
    ...
    try {
      ...
      debugPrint('Error loading notifications in student app bar: $e');
      if (mounted) {
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return InkWell(
                          onTap: () {
                            ...
                            MainScreen.navigateTo(context, 6); // Announcements tab for Student
                          },
                          child: Padding(
                            padding: EdgeInsets.all(12.r),
                            child: Column(
