...
async function main() {
  console.log('🌱 Starting fresh realistic seed with real name emails...');
  ...
  const generateEmail = (first, last) => {
    ...
    let email = `${base}@edusphere.edu`;
    let counter = 1;
    while (usedEmails.has(email)) {
      email = `${base}${counter}@edusphere.edu`;
      counter++;
    }
  ...
  console.log('🏫 2. Creating Classes 1 to 12 (A & B Sections) and Subjects...');
  const sectionsList = [];
  const classesList = [];
  const subjectsList = [];
  ...
  // Add the 6 test users from seed-test-users.js with password 'edusphere' as well
  const addGmailUser = (email, role, firstName, lastName) => {
  ...
  addGmailUser('edusphereadmin@gmail.com', 'ADMIN', 'Test', 'Admin');
  addGmailUser('edusphereaccountant@gmail.com', 'ACCOUNTANT', 'Test', 'Accountant');
  addGmailUser('edusphereparent@gmail.com', 'PARENT', 'Test', 'Parent');
  addGmailUser('eduspheretransportmanager@gmail.com', 'STAFF', 'Test', 'Transport Manager');

  // 4. Create 50 Teachers
  console.log('👨‍🏫 4. Generating 50 Teachers with real name emails...');
  const teachersData = [];
  const teacherUserIds = [];

  // Seed the test gmail teacher
  const gmailTeacherUserId = addGmailUser('edusphereteacher@gmail.com', 'TEACHER', 'Test', 'Teacher');
  if (gmailTeacherUserId) {
    const teacherId = randomUUID();
    teachersData.push({
      id: teacherId,
      userId: gmailTeacherUserId,
      employeeId: 'TCH-GMAIL-1000',
      joiningDate: new Date('2021-06-01'),
      qualification: 'M.Sc, B.Ed',
      specialization: 'Physics'
    });
    teacherUserIds.push(gmailTeacherUserId);
  }
  ...
  console.log('🎓 5. Generating 500 Students with real name emails distributed across classes...');
  const studentsData = [];

  // Seed the test gmail student
  const gmailStudentUserId = addGmailUser('eduspherestudent@gmail.com', 'STUDENT', 'Test', 'Student');
  if (gmailStudentUserId) {
    const studentId = randomUUID();
    const section = sectionsList[0]; // Class 1 Section A
    studentsData.push({
      id: studentId,
      userId: gmailStudentUserId,
      admissionNumber: 'ADM-GMAIL-1000',
      rollNumber: '01',
      academicYearId: academicYear.id,
  ...
  // 7. Establish Connections (Class Teachers & Subject Teachers)
  console.log('🔗 6. Connecting Teachers to Classes and Subjects...');
  
  // Assign class teachers for Classes 1 to 12
  const dbTeachers = await prisma.teacher.findMany({ select: { id: true } });
  for (let i = 0; i < classesList.length; i++) {
    await prisma.class.update({
      where: { id: classesList[i].id },
      data: { classTeacherId: dbTeachers[i].id }
    });
  }
  console.log('   - Linked Class Teachers to Classes');

  // Assign Subject Teachers for all subjects in all classes
  const dbSubjects = await prisma.subject.findMany({ select: { id: true } });
  const subjectTeacherData = [];
  for (let i = 0; i < dbSubjects.length; i++) {
    const randomTeacher = dbTeachers[i % dbTeachers.length];
  ...
  await prisma.subjectTeacher.createMany({ data: subjectTeacherData });
  console.log(`   - Linked ${subjectTeacherData.length} Subjects to Teachers`);

  // 8. Seed to Supabase Auth
  console.log('🔑 7. Seeding authentication credentials to Supabase auth.users...');
  ...
  console.log('\n🎉 ALL FRESH DATA SEEDING COMPLETED SUCCESSFULLY!');
}
