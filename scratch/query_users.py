...
url = "https://bstevdkjqjzaglayicdg.supabase.co/rest/v1/User?select=id,email,role&limit=20"
headers = {
...
if response.status_code == 200:
    for idx, user in enumerate(response.json(), 1):
        print(f"{idx}. Email: {user.get('email')}, Role: {user.get('role')}")
else:
    print(response.text)
