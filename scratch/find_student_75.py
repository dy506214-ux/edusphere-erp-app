...
def main():
    ...
    try:
        ...
        print("\n--- Students with 3 Present & 1 Absent in June 2026 ---")
        cursor.execute("""
            ...
            HAVING COUNT(CASE WHEN att.status::text IN ('PRESENT', 'P', 'LATE', 'Late', 'HALF_DAY') THEN 1 END) = 3
               AND COUNT(CASE WHEN att.status::text IN ('ABSENT', 'A') THEN 1 END) = 1;
        """)
